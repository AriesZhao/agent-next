...
def _ensure_data_source_schema(db: DatabasePool) -> None:
    """Create the six NL2SQL data-source tables (schema v16) and the
    ``agents.data_source_ids`` profile column. Idempotent for both dialects.
    """
    if not _table_exists(db, "users"):
        return
    is_pg = db.dialect == "postgresql"
    pk = (
        "BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"
        if is_pg
        else "INTEGER PRIMARY KEY AUTOINCREMENT"
    )
    ts_type = "BIGINT" if is_pg else "INTEGER"
    blob_type = "BYTEA" if is_pg else "BLOB"
    owner_type = "BIGINT" if is_pg else "INTEGER"
    stmts = [
        f"""CREATE TABLE IF NOT EXISTS data_sources (
              id {pk},
              data_source_id TEXT NOT NULL UNIQUE,
              owner_user_id {owner_type} NOT NULL REFERENCES users(id) ON DELETE CASCADE,
              name TEXT NOT NULL,
              description TEXT NOT NULL DEFAULT '',
        ...
        "CREATE INDEX IF NOT EXISTS idx_data_sources_shared ON data_sources(shared) "
        "WHERE shared = 1",
        f"""CREATE TABLE IF NOT EXISTS data_source_connections (
              ...
              password_blob {blob_type},
              ssl_mode TEXT NOT NULL DEFAULT '',
              last_tested_at {ts_type}
            )""",
        ...
        f"""CREATE TABLE IF NOT EXISTS data_source_schemas (
              data_source_id TEXT PRIMARY KEY
                REFERENCES data_sources(data_source_id) ON DELETE CASCADE,
              tables_json TEXT NOT NULL DEFAULT '[]',
              refreshed_at {ts_type} NOT NULL DEFAULT 0
            )""",
        f"""CREATE TABLE IF NOT EXISTS data_source_sql_audit (
              id {pk},
              data_source_id TEXT NOT NULL,
              actor_user_id {owner_type},
              thread_id TEXT,
              agent_id TEXT,
    ...
    if _table_exists(db, "agents"):
        _ensure_column(db, "agents", "data_source_ids", "TEXT")
...
def _apply_sqlite_migration(db: DatabasePool, version: int, path: Path) -> None:
    ...
    Version 16 adds NL2SQL data-source tables and ``agents.data_source_ids``.
    """
    if version == 2:
